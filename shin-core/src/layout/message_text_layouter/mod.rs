...
mod tests {
    ...
    // the tests that are follow are snapshot tests, with snapshots being extracted from the actual game engine running
    // this way we can be sure that the algorithm is implemented correctly

    // TODO: add tests for:
    // - novel mode without justification
    // TODO: add a more comprehensive way to test the layouter
    // It should be possible to dump the messages and their layouted versions for full chapters and run tests on that
