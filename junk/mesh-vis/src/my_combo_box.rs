...
pub struct ComboBox<'a, T, Message, Theme = iced_widget::Theme, Renderer = iced_widget::Renderer>
where
    ...
    Renderer: text::Renderer,
{
            ...
            self.state.with_inner(|state| {
                ...
                }) = event
                {
                    match (named_key) {
                        ...
                        key::Named::ArrowDown if !modifiers.shift() => {
                            if let Some(index) = &mut menu.hovered_option {
    ...
    fn operate(
        ...
        operation.container(None, layout.bounds(), &mut |operation| {
            struct OperationWrapper<'a, Message> {
            ...
            impl<'a, NewMessage, Message> Operation<NewMessage> for OperationWrapper<'a, Message> {
                fn container(
                    ...
                    _bounds: Rectangle,
                    _operate_on_children: &mut dyn FnMut(&mut dyn Operation<NewMessage>),
                ) {
                    todo!()
                }
                ...
                fn finish(&self) -> widget::operation::Outcome<NewMessage> {
                    todo!()
                }
            }
            ...
            self.text_input.operate(
                ...
                layout,
                renderer,
                &mut OperationWrapper { operation },
            )
        })
    }
}
