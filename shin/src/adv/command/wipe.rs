...
#[derive(Debug)]
enum WipeState {
    WaitingForWipeLoader { load_task: Option<Task<AnyWiper>> },
    WaitingForWipe,
    Finished,
}
...
impl StartableCommand for command::runtime::WIPE {
    ...
    fn start(
        ...
        let load_task = if self.duration > Ticks::ZERO {
            ...
            Some(shin_tasks::IoTaskPool::get().spawn(async move {
                AnyWiper::load(
                    &asset_server,
                    &scenario,
                    self.ty,
            ...
            replace_with::replace_with(
                ...
                |state| match state {
                    WipeState::WaitingForWipeLoader { load_task } => match load_task {
                        Some(load_task) if load_task.is_finished() => {
                            let wiper = shin_tasks::block_on(load_task);
